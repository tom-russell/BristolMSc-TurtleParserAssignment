...
int main(int argc, char **argv)
{
	...
	/* Wait until the user has closed the SDL window */
	while (!sw.finished) {
		Neill_SDL_Events(&sw);
	}
	
	/* Clear up memory & graphics subsystems before exit */
	FreeMemory(&prog);
	atexit(SDL_Quit);
...
int CountTokens(FILE *fp)
{
	...
	while (fscanf(fp, "%s", string) == 1) {
		if (strlen(string) > MAXTOKENLEN) {
			fprintf(stderr, "Token %d exceeds max token length.\n", count + 1);
			exit(token_len_error);
		}
		count++;
	}
	/*Return to the start of the file to start copying the tokens */
	rewind(fp);
...
{
	/* If the next token is not the expected token, then the file is invalid */
...
double PopStack(P_stack *stack, Program *prog)
{
	...
	if (stack->top < 0) {
		ParseError("Invalid Polish expression. Too many or incorrectly placed "
		           "operators.", prog);
	}
...
void TurtleMove(SDL_Simplewin *sw, int xy_vals[2][2])
{
	...
	SDL_RenderDrawLine(sw->renderer, xy_vals[0][0], xy_vals[0][1], 
	...
	/* Check if the user has attempted to quit the program */
	if (sw->finished) {
		atexit(SDL_Quit);
		exit(0);
	}
...
{
    /* Add the current turn to the angle */
